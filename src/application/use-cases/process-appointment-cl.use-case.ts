...
export class ProcessAppointmentClUseCase implements UseCase<CountryAppointmentSQSEventDto, void> {
  ...
  async execute(input: CountryAppointmentSQSEventDto): Promise<void> {
    try {
      ...
      // Ensure table exists for Chile
      await this.rdsService.createTableIfNotExists('CL');
